// The standalone broker: a long-running server that pairs whoever dials in.
/*
bazel run //game_mcts/tournament_server/referee:broker_server -- \
    --grpc_port=50051 --http_port=8080 --data_dir=tournament_data
*/
//
// This is the local development loop, and the only place games are played
// outside a sandbox. Point dev_bot, random_client or risk_mcts_client at it and
// iterate without an arena, a fleet or a container in the way.
//
// It is not on the arena's evaluation path. A rated match runs under
// :match_referee, started per order by a worker on a private network, so the
// coordinator links no game code. The difference between the two binaries is
// only when they stop: this one serves until killed, that one plays a fixed
// number of games and reports a tally.
//
// Same caveat as ever: identity here is a bare player name over insecure gRPC.
// Anything that can reach this port can play as any name.
...
auto main(int argc, char **argv) -> int {
  ...
  // No submission registry here, so the board ranks whoever has actually
  // played -- ad-hoc clients and builtins included. That is what a dev loop
  // wants to see; the arena's board is about submissions.
  tournament_arena::EloStandings standings(&elo_store, /*candidates=*/nullptr,
                                           "");
  tournament_broker::HttpLeaderboard leaderboard(
      absl::GetFlag(FLAGS_http_port), &history, /*candidates=*/nullptr,
      &standings, "Broker");
  ...
  LOG(INFO) << "Broker on :" << absl::GetFlag(FLAGS_grpc_port)
            << ", leaderboard on http://localhost:"
            << absl::GetFlag(FLAGS_http_port) << ", data dir " << data_dir;
