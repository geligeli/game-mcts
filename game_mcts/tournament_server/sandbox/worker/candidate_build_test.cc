...
TEST(GenerateCandidateBuildTest, WiresTheEntryHeaderAndGameIntoTheBinary) {
  ...
  EXPECT_NE(build.find("//game_mcts/tournament_server/candidate_api:candidate_main.cc"),
            std::string::npos);
  EXPECT_NE(
      build.find(
...
TEST(GenerateCandidateBuildTest, IncludesAllowedExtraDeps) {
  proto::WorkOrder order = MakeOrder();
  order.add_extra_deps("//game_mcts/games/risk/strategies:risk_proposer");
  order.add_extra_deps("//game_mcts/core/mcts:mcts");

  const std::string build = GenerateCandidateBuild(order);
  EXPECT_NE(build.find("//game_mcts/games/risk/strategies:risk_proposer"),
            std::string::npos);
  EXPECT_NE(build.find("//game_mcts/core/mcts:mcts"),
            std::string::npos);
  EXPECT_NE(build.find("//game_mcts/tournament_server/candidate_api:candidate_api"),
            std::string::npos);
}
