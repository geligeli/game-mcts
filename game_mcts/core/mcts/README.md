# MCTS framework (`game_mcts/core/mcts/`)
...
`game_mcts/games/tictactoe/` and `game_mcts/games/pig/` (see
`game_mcts/README.md` for an overview). A full-size stochastic game
(Risk) is built on this framework in `game_mcts/games/risk/`, and is the source
...
## The contracts (`game_mcts/core/mcts/game_traits.h`)
...
`risk_game::RiskProposer` (`game_mcts/games/risk/strategies/risk_proposer.h`).
...
- `mcts::tournament::TournamentPolicy<G>`: `policy(game, gen) ->
  ...
  search; used by the tournament runner (`game_mcts/core/mcts/tournament.h`).

## The game runner (`game_mcts/core/mcts/game_runner.h`)
...
## MCTS (`game_mcts/core/mcts/mcts.h`, implementations in `mcts.inl`)
...
This is the idiom used per move in `game_mcts/games/risk/risk_mcts_selfplay.cpp`:
...
- `ExportTree(runner)` (`game_mcts/core/mcts/mcts_export.h`) exports the full tree to a
...
The recipe below follows `game_mcts/games/tictactoe/` (deterministic) and notes where
`game_mcts/games/pig/` and `game_mcts/games/risk/` diverge.

### 1. Define the state class

Create `game_mcts/games/mygame/mygame.h`. The class is an immutable-ish value type: the
...
Add, as in `game_mcts/games/pig/pig_game.h`:
...
`game_mcts/games/mygame/BUILD`, following `game_mcts/games/tictactoe/BUILD`:
...
cc_library(
    ...
    srcs = ["mygame.cpp"],
    hdrs = ["mygame.h"],
    deps = ["//game_mcts/core/mcts:game_traits"],
)
...
Verify: `bazel test //game_mcts/games/mygame:mygame_test`.
...
`mygame_serialization.h` (pattern: `game_mcts/games/tictactoe/tictactoe_serialization.h`):
...
`mygame_serialization` library depending on `//game_mcts/core/mcts:serialization`.
...
from Python through one game-agnostic interface. `game_mcts/core/python/py_game.h` provides
...
`game_mcts/core/python/py_game_binding.h` binds that interface once per module
(`mcts::BindPyGameClass(m)`).

A game module is then a few lines (pattern: `game_mcts/games/risk/risk_pybind.cpp`):
...
`//game_mcts/core/python:py_game_binding`). On the Python side, construct the game's proto
messages with the `py_proto_library` bindings and pass
`SerializeToString()`/`FromString()` across the boundary — see
`game_mcts/games/risk/risk_engine_test.py`.

MCTS is exposed the same way for search debugging and policy evaluation.
`game_mcts/core/python/py_mcts.h` provides `mcts::PyMcts` / `mcts::MakePyMcts<G>(root,
...
```cpp
#include "game_mcts/core/mcts/mcts.inl"
#include "game_mcts/games/mygame/mygame.h"
...
`cc_binary` target depending on `:mygame`, `//game_mcts/core/mcts:mcts`,
`@abseil-cpp//absl/flags:flag` and `:parse`, then
`bazel run //game_mcts/games/mygame:compute_move -- --iterations=5000`.
...
chance nodes directly. `game_mcts/games/risk/risk_mcts_selfplay.cpp` is the reference
...
```
bazel run @risk_game_ai//game_mcts/games/risk:risk_mcts_selfplay -- --iterations_per_move=100 --delay_ms=200
```
...
`game_mcts/core/mcts/tournament.h` runs a multithreaded round-robin with ELO ratings.
...
`game_mcts/games/risk/risk_tournament.cpp` is the reference binary, driven by a config
file (see `game_mcts/games/risk/example_tournament.cfg`).

### 6. Benchmarking

`game_mcts/core/mcts/mcts_bench.cpp` (google-benchmark) times MCTS on TicTacToe;
`game_mcts/games/risk/risk_game_benchmark.cpp` does the same for Risk transitions and
