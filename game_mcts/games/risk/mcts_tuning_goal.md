...
Done when: `bazel run -c opt //game_mcts/games/risk:risk_tournament` with a fixed seed and at
...
final config + standings are saved to `games/risk/tuning_result.md`.
...
`bazel test //game_mcts/games/risk:all @game_mcts//...` after any code change.

Scope: only `games/risk/` in this repo (framework tweaks belong in the
