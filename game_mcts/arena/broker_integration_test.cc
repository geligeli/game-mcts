...
TEST_F(BrokerIntegrationTest, TwoClientsPlayFullGame) {
  ...
  for (const auto &entry :
       std::filesystem::directory_iterator(dir_ / "games")) {
...
TEST_F(BrokerIntegrationTest, TypedPolicyClientVsBuiltin) {
  ...
  auto future_g = std::async(std::launch::async, [&] {
    ...
    return PlayRemoteGames<TicTacToe>(st.get(), "grace", "tictactoe", "any", 1,
                                      random_policy, gen_g);
  });
  std::mt19937 gen_h(8);
...
TEST_F(BrokerRendezvousTimeoutTest,
       ParkedClientIsReleasedWhenPartnerNeverComes) {
  std::mt19937 gen(36);
...
TEST_F(BrokerTimeBudgetTest, FastClientIsUnaffectedByTheBudget) {
  std::mt19937 gen(38);
  const ClientResult result =
      RunRandomClient("sprinter", "builtin:random", &gen);
  ASSERT_TRUE(result.got_game_over);
  EXPECT_EQ(result.reason, "normal");
}
...
TEST_F(BrokerIntegrationTest, LeaderboardServesRatingsAndGames) {
  ...
  // No submission registry: the broker's board ranks whoever has played, which
  // is what a dev loop wants to see.
  tournament_arena::EloStandings standings(elo_store_.get(),
                                           /*candidates=*/nullptr, "tictactoe");
  HttpLeaderboard http(/*port=*/0, history_.get(), /*candidates=*/nullptr,
                       &standings, "Broker");
  ...
  // The page is titled after whatever it is a board for, so assert on the rows
  // rather than the heading.
  const std::string html = http_get("/");
  EXPECT_NE(html.find("200 OK"), std::string::npos);
  EXPECT_NE(html.find("erin"), std::string::npos) << html;
  EXPECT_NE(html.find("builtin:minimax"), std::string::npos) << html;
...
TEST_F(HelloDeadlineTest, PromptClientIsUnaffected) {
  std::mt19937 gen(31);
  const ClientResult result = RunRandomClient("prompt", "builtin:random", &gen);
